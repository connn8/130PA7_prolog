...
% available_at(X,Y) is true if the item X is in the taqueria Y
available_at(X,Y) :- taqueria(Y,_,Z), isin(X,Z).

% multi_available(X) is true if the item X is available at more than one taquria
% gets a bag of all the taqueria that item X is avalilable and checks
% that its length is greater than one
multi_available(X) :- bagof(Y, available_at(X,Y), Z), Z = [_,_|_].

% overworked(X) is true if the person X works at more than one taqueria.
% helper predicate worked_at(X,Y) is true if person X works at taqueria Y
worked_at(X,Y) :- taqueria(Y,Z,_), isin(X,Z).
overworked(X) :- bagof(Y, worked_at(X,Y), Z), Z = [_,_|_].

% total_cost(X,K) is true if the sum of the costs of the ingredients of item X
% is equal to K. 
% helper predicate sum_cost([H|T],C) is true if C is the sum of the costs of 
% items in [H|T]
sum_cost([],0).
sum_cost([H|T],C) :- cost(H,P), sum_cost(T,C1), C is C1 + P. 
total_cost(X,K) :- ingredients(X,L), sum_cost(L,C), K = C. 

% has_ingredients(X,L) is true if the item X has all the ingredients listed in L
has_ingredients(X,L) :- ingredients(X,L1), intersection(L,L1,Y), L = Y.

% avoids_ingredients(X,L) is true if the item X does not have any of the 
% ingredients listed in L
avoids_ingredients(X,L) :- ingredients(X,L1), intersection(L,L1,Y), Y = [].

% p1(L,X) is true if L contains all of the items that have all of the ingredients 
% listed in X
p1(L,X) :- bagof(Y, has_ingredients(Y,X), L). 

% p2(L,Y) is true if L contains all of the items that do not have any of the 
% ingredients listed in Y  
p2(L,Y) :- bagof(X, avoids_ingredients(X,Y), L).

% find_items(L,X,Y) is true if L is the list of all the items that contain all of 
% the ingredients in X and do not contain any of the ingredients in Y
